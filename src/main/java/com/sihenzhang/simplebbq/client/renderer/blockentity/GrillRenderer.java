    ...
    public void render(GrillBlockEntity pBlockEntity, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBufferSource, int pPackedLight, int pPackedOverlay) {
        ...
        for (var j = 0; j < inventory.getSlots(); j++) {
            ...
            if (!itemStack.isEmpty()) {
                ...
                Minecraft.getInstance().getItemRenderer().renderStatic(itemStack, ItemTransforms.TransformType.FIXED, pPackedLight, pPackedOverlay, pPoseStack, pBufferSource, posInt + j);
                pPoseStack.popPose();
            }
        }
        ...
        pPoseStack.pushPose();
        blockRenderer.renderSingleBlock(blockState, pPoseStack, pBufferSource, pPackedLight, pPackedOverlay);
        pPoseStack.popPose();
    }
}
